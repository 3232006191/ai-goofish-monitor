...
<template>
  <form id="task-form" @submit.prevent="handleSubmit">
    <div class="grid gap-6 py-4">
      ...
      <div class="grid gap-2 sm:grid-cols-4 sm:items-center sm:gap-4">
        <Label for="auto-compare" class="sm:text-right">{{ t('tasks.form.autoCompare') }}</Label>
        <div class="space-y-1 sm:col-span-3">
          <Switch id="auto-compare" v-model="form.auto_compare_enabled" />
          <p class="text-xs text-gray-500">
            {{ t('tasks.form.autoCompareHint') }}
          </p>
        </div>
      </div>
      <div class="grid gap-2 sm:grid-cols-4 sm:items-center sm:gap-4">
