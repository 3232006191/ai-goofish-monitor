...
from src.services.paijitang_scraper import run_comparison
...
async def scrape_xianyu(task_config: dict, debug_limit: int = 0):
    ...
    # 自动比价：如果任务开启了自动比价，触发拍机堂搜索
    if _as_bool(task_config.get("auto_compare_enabled"), False):
        keyword = task_config.get("keyword", "")
        if keyword:
            try:
                print(f"[自动比价] 触发拍机堂比价，关键词: {keyword}")
                compare_result = await run_comparison(
                    keyword=keyword, force_refresh=True
                )
                print(
                    f"[自动比价] 完成，找到 {compare_result.get('result_count', 0)} 条拍机堂商品"
                )
            except Exception as e:
                print(f"[自动比价] 执行失败: {e}")
