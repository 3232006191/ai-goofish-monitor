...
SCHEMA_STATEMENTS = (
    ...
    CREATE TABLE IF NOT EXISTS tasks (
        ...
        auto_compare_enabled INTEGER NOT NULL DEFAULT 0
    )
    """,
    """
...
def init_schema(conn: sqlite3.Connection) -> None:
    ...
    _migrate_tasks_auto_compare(conn)
    conn.commit()


def _migrate_tasks_auto_compare(conn: sqlite3.Connection) -> None:
    """为 tasks 表添加 auto_compare_enabled 列（仅执行一次）。"""
    row = conn.execute(
        "SELECT value FROM app_metadata WHERE key = 'migration:tasks_auto_compare'"
    ).fetchone()
    if row is not None:
        return
    cols = [r[1] for r in conn.execute("PRAGMA table_info(tasks)").fetchall()]
    if "auto_compare_enabled" not in cols:
        conn.execute(
            "ALTER TABLE tasks ADD COLUMN auto_compare_enabled INTEGER NOT NULL DEFAULT 0"
        )
    conn.execute(
        "INSERT OR REPLACE INTO app_metadata(key, value) VALUES ('migration:tasks_auto_compare', 'done')"
    )
